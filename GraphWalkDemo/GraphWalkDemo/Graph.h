...
namespace GraphProcessing
{
  ...
  {
  public:
    using iterator = typename std::unordered_map<Key, Sptr<Node<V>>>::iterator;

    Graph(const std::string& gName = "") : name_(gName) {}
    void name(const std::string& name) { name_ = name; }
    std::string name() { return name_; }
    void addNode(Sptr<Node<V>> pNode);
    void addNode(const Key& name);
    bool containsNode(Sptr<Node<V>> pNode);
    bool containsNode(const Key& name);
    bool addEdge(Sptr<Node<V>> pSrc, Sptr<Node<V>> pDst);
    bool addEdge(const Key& srcName, const Key& dstName);
    iterator begin() { return nodeMap_.begin(); }
    iterator end() { return nodeMap_.end(); }
    Sptr<Node<V>> find(const Key& name);
    void unmarkAll();
    std::vector<Sptr<Node<V>>> findUnMarked();
    template<typename T, typename P>
    void walker(T t, Sptr<Node<V>> pNode, P p, bool walkAll = false);
  private:
    std::unordered_map<Key, Sptr<Node<V>>> nodeMap_;
    std::string name_;
  };
  ...
  */
  template<typename V>
  template<typename T, typename P>
  void Graph<V>::walker(T t, Sptr<Node<V>> pNode, P p, bool walkAll)
  {
    ...
    if (walkAll)
    {
      std::vector<Sptr<Node<V>>> unmarked = findUnMarked();
  ...
  void Graph<V>::addNode(const Key& name)
  {
    if (containsNode(name))
      return;
    nodeMap_[name] = Sptr<Node<V>>(new Node<V>(name));
  }
  //----< add a node to the graph >------------------------------------

  template<typename V>
  void Graph<V>::addNode(Sptr<Node<V>> pNode)
  {
    addNode(pNode->name());
  }
  ...
  template<typename V>
  bool Graph<V>::containsNode(Sptr<Node<V>> pNode)
  {
  ...
  */
  template<typename V>
  Sptr<Node<V>> Graph<V>::find(const Key& name)
  {
    iterator iter = nodeMap_.find(name);
  ...
  template<typename V>
  bool Graph<V>::addEdge(Sptr<Node<V>> pSrc, Sptr<Node<V>> pDst)
  {
  ...
  template<typename V>
  std::vector<Sptr<Node<V>>> Graph<V>::findUnMarked()
  {
    std::vector<Sptr<Node<V>>> unmarked;
    for (auto item : nodeMap_)
    {
  ...
  */
  ...
  void walker(T t, P p, Graph<V> graph, Sptr<Node<V>> pNode, bool walkall = false)
  {
    t(pNode, p);
    if (walkall)
    {
      std::vector<Sptr<Node<V>>> unmarked = graph.findUnMarked();
      for (auto pNode : unmarked)
        t(pNode, p);
    }
    graph.unmarkAll();
...
void DFS_r_ltr(Sptr<Node<V>> pNode, P p)
{
  if (pNode->marked())
    return;
  pNode->mark();
...
template<typename V, typename P>
void DFS_r_ltrbtt(Sptr<Node<V>> pNode, P p)
{
  if (pNode->marked())
    return;
  pNode->mark();
...
template<typename V, typename P>
void DFS_r_rtl(Sptr<Node<V>> pNode, P p)
{
  if (pNode->marked())
    return;
  pNode->mark();
...
*/
template<typename V, typename P>
void DFS_s_rtl(Sptr<Node<V>> pNode, P p)
{
  std::stack<Sptr<Node<V>>> nodeStack;
  nodeStack.push(pNode);
  while (nodeStack.size() > 0)
  {
    Sptr<Node<V>> pCurrentNode = nodeStack.top();
    nodeStack.pop();
    pCurrentNode->mark();
    p(pCurrentNode);
...
template<typename V, typename P>
void DFS_s_ltr(Sptr<Node<V>> pNode, P p)
{
  std::stack<Sptr<Node<V>>> nodeStack;
  nodeStack.push(pNode);
  while (nodeStack.size() > 0)
  {
    ...
    for (size_t i = 0; i < numChildren; ++i)
    {
      Sptr<Node<V>> pChild = pCurrentNode->children()[numChildren - i - 1];
      if (pChild->marked())
        continue;
      nodeStack.push(pChild);
    }
...
*/
template<typename V, typename P>
void BFS_q(Sptr<Node<V>> pNode, P p)
{
  std::queue<Sptr<Node<V>>> nodeQueue;
  nodeQueue.push(pNode);
  while (nodeQueue.size() > 0)
  {

    Sptr<Node<V>> pCurrentNode = nodeQueue.front();
    nodeQueue.pop();
    pCurrentNode->mark();
    p(pCurrentNode);
    for (auto pChild : pCurrentNode->children())
